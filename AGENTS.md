...
- **A routine belongs to an agent, not a ticket.** It is created conversationally when the person asks that agent for repeated work, or written by hand in the Inbox **Routines** tab, and managed in either. Its cadence is daily, weekdays, weekly, monthly, or an interval in minutes — `MIN_INTERVAL_MINUTES` is the floor, because the runner wakes once a minute. An interval is anchored on local midnight rather than on the last run, so a quarter-hour routine lands on the quarter hour instead of drifting a tick later each time. Its instruction is typed or read from a markdown file on every run (`promptPath`), which only the person may set — a path an agent could set is a way to read any file into a prompt. A run lands in the agent's Inbox conversation and is **skipped, not queued**, while that conversation is working, so a routine never piles up behind a turn and never lands on another machine instead. The machine on the create event owns the clock so paired machines do not trigger it twice; each trigger tries the current device preference order and sends the prompt to the first device that can run the agent. It writes no ticket and needs no workspace. The old `recurrences` projection remains only as compatible storage, and older recurring-ticket events stay inert rather than becoming routines.
- **Directives are what travels into a task.** `start_thread` hands a new thread nothing but its prompt, so an agent handing work to another cannot pass along how to build or how to test. An agent's `directives` — typed, or read from a markdown file on every turn — are injected by the daemon into any thread running as that agent, never into its own Inbox conversation, where `instructions` already do the work. They ride in the per-turn context array in `chat.ts`, which is the one seam all three providers read; the per-session system prompt is three different mechanisms and Cursor's does not survive a resume. `LOCKED` keeps them off the built-in agent, and no agent can set them because the capability allowlist has no `PATCH /agents/:id`. They are **not redacted**, so credentials and environment values must never go in them.
