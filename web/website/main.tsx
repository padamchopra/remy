...
const guides = [
  ["web", "Use Remy on the web", "Open Remy in your browser and use your email to create an account or sign in; Google and GitHub are optional. Open the link in your email within five minutes. An organization is optional. In Threads, add a repository workspace, then use Set up a computer. Choose Cloud to configure cloud execution, or General to find Mac installation instructions. The + button opens the Mac setup flow; connected computers have their own settings panes. On your Mac, open Settings → Computers and choose Attach this Mac, approve its matching code in your browser using the same account, then choose Finish connecting on the Mac. Return to Threads and describe what you want done; Remy sends that request when the thread opens. Fly Sprites manages resources automatically; Modal offers Light, Standard, and Heavy sizes with rough hourly compute estimates and custom settings available. For Codex, select a workspace, prepare its Codex connection and sign in to your ChatGPT account using a device code, or use an OpenAI API key. Claude uses an Anthropic API key. Create or join an organization when you want to work with other people."],
