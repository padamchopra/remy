...
const guides = [
  ...
  ["providers", "Bring your provider", "On your Mac, install and sign in to Claude Code, Codex, or Cursor Agent using that provider’s own instructions. On an available hosted computer, connect Codex with a ChatGPT device code or OpenAI API key; Claude uses an Anthropic API key. Separate task computers use short-lived tokens from the workspace’s saved Codex connection; the connection computer retains its refresh credentials. In Remy, choose the provider and model when you start a thread. Provider subscriptions and usage charges are separate from Remy."],
...
function Changelog() {
  return <main className="site-section changelog-page"><p className="eyebrow">CHANGELOG</p><h1>Remy keeps moving.</h1><p>New capabilities, useful improvements, and fixes worth knowing about.</p><article><span className="release-label">Unreleased</span><h2>A closer look at Remy.</h2><p>Explore the website’s interactive demo, powered by the same interface components as the app. Browse setup guides and find the latest Mac download.</p><ul><li>Mark an agent available as a subagent, then let a Claude thread hand part of its work to it; watch each one in the thread’s Running work tab.</li><li>Keep your Mac app and its background connection on the same version after updates; older installations need a one-time Mac restart.</li><li>Try local web changes with your live account through an explicit preview approval on your Mac.</li><li>Save your own Fly.io Sprites or Modal credentials in Cloud; provider credentials stay outside your computers.</li><li>Collapse the web sidebar to an icon rail using its own toggle, with labels on hover.</li><li>Open web settings from the bottom of the sidebar, with a dedicated settings menu and Back to return to your work.</li><li>Set up a computer before adding your first web workspace, with guidance when your computer is unavailable.</li><li>Open Computers to configure Cloud first, or choose Connected to see and add your computers.</li><li>Create an account or sign in with an email link, without Google or GitHub.</li><li>Sign in with a clearer form, recognizable provider icons, and more room on smaller screens.</li><li>Follow clearer web setup actions, review who invited you before joining, and start your first thread with a request instead of a name.</li><li>Use Remy on the web with a personal account, an optional organization, and a compact sidebar that keeps your threads close.</li><li>Run cloud tasks on separate computers with a concurrency limit, and reuse environment values across local and cloud workspaces.</li><li>Configure Fly.io Sprites and Modal independently and choose an enabled provider in the computer picker.</li><li>Connect Codex on a hosted computer with your ChatGPT account using a device code, while keeping API keys available.</li><li>See skeletons and progress indicators while the web app opens.</li><li>Select sample threads and try the composer.</li><li>Explore worktrees, agents, and reviewing changes.</li><li>Open the web app and find setup instructions for hosted computers, your Mac, and other devices.</li></ul></article><a className="text-link" href={`${repo}/releases`}>View released app versions on GitHub<ArrowUpRight /></a></main>;
}
function Footer() {
