...
const guides = [
  ["web", "Use Remy on the web", "Open Remy in your browser and sign in to your personal account; an organization is optional. Add a repository workspace and configure an available hosted computer with your provider credentials in Computers, or connect your Mac. Fly Sprites manages resources automatically; Modal offers Light, Standard, and Heavy sizes with rough hourly compute estimates and custom settings available. For Codex, select a workspace, start its computer and connect your ChatGPT account using a device code, or use an OpenAI API key. Claude uses an Anthropic API key. Create or join an organization when you want to work with other people."],
  ...
  ["providers", "Bring your provider", "On your Mac, install and sign in to Claude Code, Codex, or Cursor Agent using that provider’s own instructions. On an available hosted computer, connect Codex with a ChatGPT device code or OpenAI API key; Claude uses an Anthropic API key. Threads on that hosted computer use its connected account. In Remy, choose the provider and model when you start a thread. Provider subscriptions and usage charges are separate from Remy."],
...
function Changelog() {
  return <main className="site-section changelog-page"><p className="eyebrow">CHANGELOG</p><h1>Remy keeps moving.</h1><p>New capabilities, useful improvements, and fixes worth knowing about.</p><article><span className="release-label">Unreleased</span><h2>A closer look at Remy.</h2><p>Explore the website’s interactive demo, powered by the same interface components as the app. Browse setup guides and find the latest Mac download.</p><ul><li>Use Remy on the web with a personal account, an optional organization, and a compact sidebar that keeps your threads close.</li><li>Choose a hosted computer size with optional customization; Fly Sprites manages resources automatically.</li><li>Connect Codex on a hosted computer with your ChatGPT account using a device code, while keeping API keys available.</li><li>See skeletons and progress indicators while the web app opens.</li><li>Select sample threads and try the composer.</li><li>Explore worktrees, agents, and reviewing changes.</li><li>Open the web app and find setup instructions for hosted computers, your Mac, and other devices.</li></ul></article><a className="text-link" href={`${repo}/releases`}>View released app versions on GitHub<ArrowUpRight /></a></main>;
}
function Footer() {
