...
const guides = [
  ...
  ["providers", "Bring your provider", "On your Mac, install and sign in to Claude Code, Codex, or Cursor Agent using that provider’s own instructions. On an available hosted computer, connect Codex with a ChatGPT device code or OpenAI API key; Claude uses an Anthropic API key. OpenRouter support is unreleased: save its API key under Computers → Model access, then choose a model when starting a thread once your service is updated. Separate task computers use short-lived tokens from the workspace’s saved Codex connection; the connection computer retains its refresh credentials. In Remy, choose the provider and model when you start a thread. Provider subscriptions and usage charges are separate from Remy."],
