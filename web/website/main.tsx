...
const faqs = [
  ["What do I need to run Remy?", "Use Remy in your browser with a hosted computer and your organization’s provider credentials. For local use, install Remy and a coding provider on a Mac that stays awake while your threads run."],
  ["Can I use Remy without installing the Mac app?", "Yes. Open Remy in your browser, sign in, and use a hosted computer. Your organization needs hosted computers enabled and provider credentials configured."],
...
const guides = [
  ["web", "Use Remy on the web", "Open Remy in your browser and sign in to your organization. Add a repository workspace and choose a hosted computer to run threads without installing the Mac app. An organization administrator configures hosted computers and provider credentials in Computers. You can also use your organization’s connected computers."],
...
function Changelog() {
  return <main className="site-section changelog-page"><p className="eyebrow">CHANGELOG</p><h1>Remy keeps moving.</h1><p>New capabilities, useful improvements, and fixes worth knowing about.</p><article><span className="release-label">Unreleased</span><h2>A closer look at Remy.</h2><p>Explore the website’s interactive demo, powered by the same interface components as the app. Browse setup guides and find the latest Mac download.</p><ul><li>See loading placeholders while the web app opens and a clearer organization setup sidebar in Remy for Teams.</li><li>Select sample threads and try the composer.</li><li>Explore worktrees, agents, and reviewing changes.</li><li>Open the web app and find setup instructions for hosted computers, your Mac, and other devices.</li></ul></article><a className="text-link" href={`${repo}/releases`}>View released app versions on GitHub<ArrowUpRight /></a></main>;
}
function Footer() {
