...
const faqs = [
  ...
  ["Can I use Remy without installing the Mac app?", "Yes. Open Remy in your browser, sign in, and use a hosted computer. Choose Cloud in Computers to enable hosted computers and configure your provider credentials; hosted availability depends on your Remy service."],
...
const guides = [
  ["web", "Use Remy on the web", "Open Remy in your browser and create an account with your email and password, or sign in with that password or a link we email you. Google and GitHub are optional. A sign-in link works for five minutes. An organization is optional. In Threads, use Set up a computer to connect your Mac or configure cloud execution, then add a repository workspace. Open Settings at the bottom of the sidebar to find Computers and other settings; Back returns to your work. Computers opens Cloud by default; choose Connected to see your computers. In Cloud, save your Fly.io Sprites, Modal, or Cursor Cloud credentials and enable any of them; each connection has its own switch. The preferred provider controls placement for new computers. Choose Add computer for Mac installation instructions. Select a computer to open its settings. On your Mac, open Settings → Computers and choose Attach this Mac, approve its matching code in your browser using the same account, then choose Finish connecting on the Mac. Return to Threads and describe what you want done; Remy sends that request when the thread opens. Fly Sprites manages resources automatically; Modal offers Light, Standard, and Heavy sizes with rough hourly compute estimates and custom settings available. Cursor Cloud runs on Cursor-hosted VMs and clones the workspace from its git remote. For Codex, select a workspace, prepare its Codex connection and sign in to your ChatGPT account using a device code, or use an OpenAI API key. Claude uses an Anthropic API key. Create or join an organization when you want to work with other people."],
  ...
  ["environments", "Reuse your workspace environment", "In Settings → Environments, define values and assign an environment to one or more workspaces. Tasks inherit these values on local and cloud computers. Stored values are encrypted and hidden in settings, but providers and commands can read their assigned environment. Exact redaction cannot recognise encoded or transformed values. Automatic cloud execution gives each task a separate computer, up to your configured concurrency limit; sleeping computers restore their task state when work resumes."],
  ["providers", "Bring your provider", "On your Mac, install and sign in to Claude Code, Codex, or Cursor Agent using that provider’s own instructions. On an available hosted computer, connect Codex with a ChatGPT device code or OpenAI API key; Claude uses an Anthropic API key. Cursor Cloud uses a Cursor API key and runs on Cursor-hosted VMs; local Cursor threads still use Cursor Agent on your Mac. Save an OpenRouter, Router, OpenAI, or Anthropic key under Computers → Model access, then choose that model when you start a cloud thread. Separate task computers use short-lived tokens from the workspace’s saved Codex connection; the connection computer retains its refresh credentials. In Remy, choose the provider and model when you start a thread. Provider subscriptions and usage charges are separate from Remy."],
  ["threads", "Start a thread", "Open a workspace folder, choose your model and permission mode, and describe what you want done. On the web, choose Private or Shared in the composer. Follow the response, inspect tool output, and send another message to continue."],
...
function Changelog() {
  return <main className="site-section changelog-page"><p className="eyebrow">CHANGELOG</p><h1>Remy keeps moving.</h1><p>New capabilities, useful improvements, and fixes worth knowing about.</p><article><span className="release-label">Unreleased</span><h2>A closer look at Remy.</h2><p>Explore the website’s interactive demo, powered by the same interface components as the app. Browse setup guides and find the latest Mac download.</p><ul><li>Run a web thread on Cursor Cloud from Computers, with your Cursor API key and Cloud · Cursor in the picker.</li><li>Share a computer or cloud connection with your organization and choose which providers others can start threads with.</li><li>See what a new web thread is doing as it starts, including waking or restoring a cloud computer.</li><li>Search your repositories when you add a workspace, with each one’s description, language and last push in the row, and the ones you already added marked.</li><li>Use a personal access token to reach repositories in an organization that has not installed Remy, even when GitHub is already connected.</li><li>Open Add a workspace to your repositories straight away: the list is read alongside your account rather than after it, and reopening paints the repositories you saw last while it refreshes.</li>
<li>See a toast when you invite someone or save settings, instead of a line of status text.</li><li>Create a web account with your email and password, then sign in with that password or a sign-in link.</li><li>Start a private or shared organization thread on a Fly.io computer shared into that organization, including OpenRouter from that computer.</li><li>Right-click a thread in the web sidebar to pin, rename, archive, or delete it, including when that cloud computer is idle.</li><li>Start a cloud thread with OpenRouter on that computer when that account has OpenRouter on, including the model already selected there.</li><li>Open a web thread from an address that names only that thread.</li><li>Pick a model in a new web thread from the full provider list, and keep the computer, model, and branch still while they load.</li><li>Open a workspace from All and keep its details visible after a refresh. Start a thread from its composer while the sidebar keeps existing threads close.</li><li>Choose All in the account switcher to browse one combined list of personal and organization threads, workspaces, tasks, agents, computers, environments, connections, and routing rules. Choose one account to narrow that same view. New threads open directly in the composer; their workspace chooses the owning account, and Private or Shared stays in the composer. Manage members and teams in Organization settings, with an explicit organization selector. Share Personal computers and cloud connections with an organization without exposing their credentials.</li><li>Keep your Mac app and its background connection on the same version after updates; older installations need a one-time Mac restart.</li><li>Choose your web profile avatar from the same presets, photo upload, and GitHub picture picker as the Mac app, and see each member’s saved avatar in organization settings.</li><li>Reply on the web with the shared Mac composer, including inline image attachments and compact send and stop controls.</li><li>Recognize providers and participants by their avatars, read consistent sidebar thread rows, and keep the working branch visible in web threads.</li><li>Read web threads beneath the same compact tab header as the Mac app, with sharing and computer details in a menu.</li><li>Open hosted threads immediately while your cloud computer starts, and retry startup failures without losing your message.</li><li>Keep Fly.io computers connected while your threads run, and recover safely after a failed start.</li><li>Keep hosted previews connected on slower networks and see a clear message when a connection fails.</li><li>Try local web changes with your live account through an explicit preview approval on your Mac.</li><li>Save your own Fly.io Sprites, Modal, or Cursor Cloud credentials in Cloud; provider credentials stay outside your computers.</li><li>Collapse the web sidebar to an icon rail using its own toggle, with labels on hover.</li><li>Open web settings from the bottom of the sidebar, with a dedicated settings menu and Back to return to your work.</li><li>Set up a computer before adding your first web workspace, with guidance when your computer is unavailable.</li><li>Open Computers to configure Cloud first, or choose Connected to see and add your computers.</li><li>Create an account or sign in with an email link, without Google or GitHub.</li><li>Sign in with a clearer form, recognizable provider icons, and more room on smaller screens.</li><li>Follow clearer web setup actions, review who invited you before joining, and start your first thread with a request instead of a name.</li><li>Use Remy on the web with a personal account, an optional organization, and a compact sidebar that keeps your threads close.</li><li>Run cloud tasks on separate computers with a concurrency limit, and reuse environment values across local and cloud workspaces.</li><li>Customize hosted workspace icons and colors, and see connected computers and cloud availability.</li><li>Choose a GitHub repository when adding a workspace, using your GitHub account or a personal access token.</li><li>Choose the starting branch in the web composer with the same branch picker as the Mac app.</li><li>Start web threads with a concrete computer or cloud provider already selected, using your routing default until you choose another.</li><li>Start web threads with your workspace model preference, then your selected computer’s default, then your account default.</li><li>Star models in the web picker and keep favorites across visits.</li><li>Save provider keys automatically, keep them when disabled, and choose a provider and model for each cloud thread.</li><li>Configure Fly.io Sprites, Modal, and Cursor Cloud independently, and keep your selected cloud computer after refresh.</li><li>Connect Codex on a hosted computer with your ChatGPT account using a device code, while keeping API keys available.</li><li>Keep a steady loading indicator while your hosted threads open, with consistent empty states across hosted screens.</li><li>Select sample threads and try the composer.</li><li>Explore worktrees, agents, and reviewing changes.</li><li>Open the web app and find setup instructions for hosted computers, your Mac, and other devices.</li></ul></article><a className="text-link" href={`${repo}/releases`}>View released app versions on GitHub<ArrowUpRight /></a></main>;
}
function Footer() {
