...
import { ArrowDownToLine, ArrowUpRight, Check, GitBranch, Github, Globe, Laptop, MessagesSquare, Repeat2, ShieldCheck, SquareKanban } from "lucide-react";
...
const features = [
  { id: "threads", tab: "Run parallel threads", title: "More going on. Less to keep in your head.", text: "Keep your threads in one view, see which ones need you, and pick up a conversation without losing your place.", icon: MessagesSquare },
  { id: "worktrees", tab: "Give each change space", title: "A separate checkout. A clearer head.", text: "Start a thread in its own git worktree so independent changes can move forward side by side.", icon: GitBranch },
  ...
  { id: "agents", tab: "Come back to your agents", title: "A conversation you can come back to.", text: "Give an agent its own instructions and model in Inbox, then ask it to take on repeated work with a routine.", icon: Repeat2 },
];
const faqs = [
...
function Navigation() {
  return <header className="site-header"><nav aria-label="Main navigation" className="site-nav"><a className="wordmark" href="/">Remy</a><div className="nav-links"><a href="/docs/">Docs</a><a href="/changelog/">Changelog</a><a href="/#features">Features</a></div><div className="nav-actions"><a className="github-link" href={repo} aria-label="Remy on GitHub"><Github /></a><Download /></div></nav></header>;
}
function DemoFrame({ scene = "threads", compact = false }: { scene?: string; compact?: boolean }) {
  const host = useRef<HTMLDivElement>(null);
  const [ready, setReady] = useState(false);
  useEffect(() => {
    const element = host.current;
    if (!element) return;
    const observer = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) { setReady(true); observer.disconnect(); } }, { rootMargin: "240px" });
    observer.observe(element);
    return () => observer.disconnect();
  }, []);
  return <div ref={host} className={compact ? "phone-demo" : "desktop-demo"}>
    {!compact && <div className="window-bar" aria-hidden="true"><span className="traffic"><i /><i /><i /></span><span>Remy</span><span className="window-note">Your workspace, your computer</span></div>}
    {ready ? <iframe key={`${scene}-${compact}`} src={`/demo/index.html?scene=${scene}${compact ? "&compact=1" : ""}`} title={compact ? "Remy browser demo in a narrow viewport" : "Interactive Remy demo"} sandbox="allow-scripts allow-same-origin allow-forms" loading="lazy" /> : <div className="demo-placeholder">Opening interactive demo…</div>}
  </div>;
}
function FeatureTabs({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <Tabs value={value} onValueChange={onChange}><TabsList aria-label="Explore Remy features">{features.map((feature) => <TabsTrigger key={feature.id} value={feature.id}>{feature.tab}</TabsTrigger>)}</TabsList></Tabs>;
}
function Home() {
  ...
  return <main>
    <section className="hero" aria-labelledby="hero-title"><p className="eyebrow"><span />Free. Local. Built around your agents.</p><h1 id="hero-title">Your agents, within reach.</h1><p className="hero-description">Run Claude Code, Codex, and Cursor on your own machines.<br className="desktop-break" /> Threads, worktrees, terminals, and reviews—on your Mac or on the go.</p><div className="hero-actions"><Download large /><Button asChild variant="outline" size="lg"><a href={repo}><Github data-icon="inline-start" />View on GitHub</a></Button></div><p className="fine-print">Also in your browser and on iPhone</p></section>
    <section className="hero-preview" aria-label="Try Remy"><FeatureTabs value={scene} onChange={setScene} /><DemoFrame scene={scene} /><p className="demo-caption">This is Remy’s real interface with sample data. Select a thread or send a message.</p></section>
    <section className="site-section explorer" id="features"><h2>From first prompt to finished work.</h2><FeatureTabs value={feature} onChange={setFeature} /><div className="explorer-layout"><div><selected.icon className="feature-icon" /><h3>{selected.title}</h3><p>{selected.text}</p><a className="text-link" href={`/docs/#${selected.id}`}>Explore {selected.id}<ArrowUpRight /></a></div><DemoFrame scene={feature} compact /></div></section>
    ...
    <section className="site-section mobile-section"><div><p className="eyebrow">PICK UP WHERE YOU LEFT OFF</p><h2>Step away.<br />Stay in the loop.</h2><p>Your Mac keeps working. Open Remy on your iPhone or in a browser to follow a thread, answer a question, or send the next prompt.</p><p>Pair your devices over your private Tailscale network.</p><Button asChild variant="outline"><a href="/docs/#pairing">Set up remote access<ArrowUpRight data-icon="inline-end" /></a></Button><p className="fine-print">Live browser demo shown beside this section.<br />The native iPhone app also connects to your Mac.</p></div><DemoFrame compact /></section>
    <section className="site-section"><h2>Everything around the conversation.</h2><p>Plan the work, give it space, and review what comes back.</p><div className="feature-grid">{[
      { icon: GitBranch, title: "Worktrees for parallel work", text: "Give each change its own branch and working folder.", link: "worktrees" },
      { icon: MessagesSquare, title: "A workbench for every thread", text: "Keep your terminal, browser, and pull request beside the conversation.", link: "review" },
      { icon: Repeat2, title: "Agents you can come back to", text: "Give an agent instructions, then ask it for recurring work in Inbox.", link: "agents" },
    ...
    ].map((item) => <a className="feature-card" key={item.title} href={`/docs/#${item.link}`}><item.icon /><h3>{item.title}<ArrowUpRight /></h3><p>{item.text}</p></a>)}</div></section>
    ...
    <section className="site-section faq"><h2>A few things<br />to know.</h2><div>{faqs.map(([question, answer]) => <Collapsible key={question}><CollapsibleTrigger className="faq-question">{question}<span aria-hidden="true">＋</span></CollapsibleTrigger><CollapsibleContent><p>{answer}</p></CollapsibleContent></Collapsible>)}</div></section>
