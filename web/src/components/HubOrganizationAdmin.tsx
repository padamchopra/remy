...
export default function HubOrganizationAdmin({organizations, selectedId, tab, onSelect, onTab}: {
  organizations:Organization[]; selectedId?:string; tab:"members"|"teams"|"computers";
  onSelect:(id:string)=>void; onTab:(tab:"members"|"teams"|"computers")=>void;
}) {
  ...
  return <HubPersonalContext value={false}>
    <section className="flex min-h-0 flex-1 flex-col overflow-auto" aria-label="Organization settings">
      ...
      <Tabs className="mt-4 min-h-0" value={tab} onValueChange={value=>onTab(value as "members"|"teams"|"computers")}>
