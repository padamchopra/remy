  ...
  return <HubPersonalContext value={false}>
    ...
    <section className="flex min-h-0 flex-1 flex-col overflow-auto" aria-label="Organization settings">
      ...
      <Tabs className="mt-4 min-h-0" value={tab} onValueChange={value=>onTab(value as OrganizationTab)}>
        ...
        <TabsContent value="general"><section className="flex flex-col gap-6 p-6" aria-label="Organization general settings"><HubModelDefault organizationId={organization.id} description="Used when a workspace or agent does not choose another model." /></section></TabsContent>
