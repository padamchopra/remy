...
async function previewRequest<T>(action: 'sign-in' | 'complete' | 'password'): Promise<T> {
...
export function HubPreviewSignIn({ password }: { password?: boolean }) {
  ...
  const run = async () => {
    ...
    try {
      if (password) {
        const result = await previewRequest<{status:string}>('password');
  ...
  return <main className="flex min-h-svh items-center justify-center p-6">
    <section className="flex w-full max-w-sm flex-col gap-5">
      ...
      <Button variant={!password && approval ? 'outline' : 'default'} disabled={busy} onClick={() => void run()}>{password ? 'Sign in' : approval ? 'Finish signing in' : 'Sign in with Remy'}</Button>
    </section>
  </main>;
}
