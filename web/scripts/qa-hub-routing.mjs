...
try {
 ...
 const startedUrl=new URL(page.url());assert.match(startedUrl.pathname,/\/threads\/[0-9a-f-]{36}$/);assert.equal(startedUrl.search,'');
 const id=startedUrl.pathname.split('/threads/')[1];const started=await call('ada',`/computers/${c.computerId}/threads/${id}`);assert.ok(started.body.detail.cwd.endsWith('android-workspace'));
