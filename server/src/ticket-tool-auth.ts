...
export function isRemyToolRoute(method: string | undefined, pathname: string): boolean {
  if (method === "POST" && /^\/organization-tools\/(list_organization_computers|list_organization_workspaces|explain_routing|start_organization_thread|create_organization_ticket|handoff_organization_ticket|move_organization_thread)$/.test(pathname)) return true;
