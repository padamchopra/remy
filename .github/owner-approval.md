...
The publisher runs on PR updates and on requested/completed review-activity workflows. It checks out only `main`, ignores contributor artifacts and workflow conclusions, and reads current PRs and paginated reviews from GitHub. Review activity has no write permission. This separation also supports reviews on fork PRs, whose workflow tokens cannot publish statuses. Each run reevaluates all open PRs because concurrent events may coalesce and a commit status can be shared by multiple PRs. Every open PR sharing a SHA must satisfy the policy.
...
Run `node --test .github/scripts/owner-approval.test.cjs`. CI exercises the same tests on every PR update. They cover author identity, exact-commit approval, revocation, live API reads, shared heads, API failure, and a head changing during evaluation.
