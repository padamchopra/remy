...
After mapping, an administrator turns sync on separately for each workspace. The initial import keeps the Linear team’s ticket slug and numbers. Existing Remy tickets become Linear issues; parent tickets become parent issues. Title, description, column, member assignment, labels, and parent relationships mirror both ways. Resolve `Work on ENG-123` in a workspace thread before sending the linked issue to its provider.
...
Sync mirrors tickets; it starts no thread of its own. A member starts the work from Remy, and that thread sends its comments and final response back with thread and artifact links. An uncertain outbound response is surfaced for inspection rather than being sent twice.
