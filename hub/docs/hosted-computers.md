...
Organization → Computers can grant an organization use of an administrator's enabled Personal cloud connection. The grant stores only the Personal scope and provider id. Provider credentials remain encrypted under the Personal scope, never appear in organization reads, and are resolved by trusted management code only when the organization allocates a computer. Start uses that source account's enabled model access, including OpenRouter, unless the organization has its own record for the same provider. Removing the grant immediately removes that provider from new organization placement without deleting the owner's connection.
