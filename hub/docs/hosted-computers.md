...
Live Fly verification awaits a configured organization. A real model conversation and first-useful-response measurement await an organization API key. The Modal sandbox has a 24-hour maximum lifetime; proactive rotation of an uninterrupted run is not yet implemented. Keep this change in draft until those gates are resolved.
