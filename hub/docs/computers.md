...
Organization → Computers lets a member share one of their Personal computers with the organization. The computer remains owned by Personal; the organization stores a revocable use grant. Other members can start organization threads with the providers the owner leaves on, and can still reply on threads already running. The computer's signing key remains on the computer, organization responses omit it, and administrators can remove the grant without gaining control of the Personal computer. Removing the computer or the member removes the grant. Cloud connections stay a binary share.
