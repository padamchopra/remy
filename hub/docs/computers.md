...
A Personal computer keeps its one authenticated connection to its Personal coordinator. Organization requests are authorized against the current grant or the owner's own membership and relayed through that coordinator with the destination organization id. Thread snapshots, manifests, attachments and notifications return to the destination coordinator, where ordinary membership, workspace and thread checks still apply. A reconnect reads the current grants plus the owner's organizations and republishes each organization's thread manifest; revocation stops relaying other members' requests immediately and tells the connected computer to stop publishing shared threads, while the owner can still start in organizations they belong to.
