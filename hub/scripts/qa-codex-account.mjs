...
export async function startHostedCodexFixture(temp, root) {
  ...
  const server = createServer(async (req,res) => {
    ...
    try {
      ...
      } else { if(req.url.endsWith('/destroy')) { connection?.stop(); const {closeHostedCodexAccount}=await import('../../server/dist/hosted-codex-account.js'); closeHostedCodexAccount(); } res.setHeader('content-type','application/json');res.end(JSON.stringify(body.runtime)); }
  ...
  return {
    ...
    close(){connection?.stop();server.close();},
  };
}
