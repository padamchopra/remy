...
Production serves the public website at `/` and the authenticated web app at `https://app.tryremy.dev/`. `npm --prefix web run build:hub` assembles both into `web/dist`; the ordinary web build is the same output a computer's own page serves. Deployments build both surfaces before publishing. The Worker serves the packaged `app/` assets on the app hostname. Existing invitations, root sign-in returns, and `/app/` links redirect to that hostname. Google and GitHub retain their registered callbacks on `tryremy.dev/api/auth/callback/*`; the Worker forwards those callbacks to the app hostname before auth processing, keeping auth cookies host-only. The app origin is explicitly trusted by Better Auth.
