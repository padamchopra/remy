...
test("replacing the Linear account hides previous mappings",async()=>{const {service,sqlite}=fixture();await service.refresh('studio','ada');const workspace=await service.organizations.createWorkspace('studio','ada',{name:'Remy',origin:'https://github.com/release/remy.git'});await service.mapWorkspace('studio','ada',workspace.id,{linearTeamId:'eng',statusMap:{todo:'todo',done:'done'}});await service.mapMember('studio','ada','u1','ada');sqlite.prepare("UPDATE organization_linear_links SET external_id='replacement' WHERE organization_id='studio'").run();await service.refresh('studio','ada');const current=await service.list('studio','ada');assert.equal(current.mappings.length,0);assert.equal(current.matches.length,0);await service.mapMember('studio','ada','u2','ada');assert.equal((await service.list('studio','ada')).matches[0].linear_user_id,'u2');});
