...
export class OrganizationService {
  ...
  private async shared(organizationId: string, userId: string) {
  ...
  async rename(organizationId: string, userId: string, name: string) { await this.shared(organizationId, userId); await this.access(organizationId, userId, ["owner", "admin"]); await this.store.renameOrganization(organizationId, name, this.now()); await this.audit(organizationId, userId, "organization.renamed", "organization", organizationId); }
  ...
  async createInvite(organizationId: string, userId: string, input: { email?: string; role: "admin" | "member" }) {
    await this.shared(organizationId, userId); await this.access(organizationId, userId, ["owner", "admin"]);
