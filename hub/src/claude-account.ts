...
/// Claude Code's public OAuth client. Cloud computers sign in the same way
/// Claude Code does on a Mac; Remy never mints a second client.
...
export function claudeAccountConnected(secrets: Record<string, string>) {
  return publicClaudeAccount(secrets).phase === "connected";
}
...
async function persist(store: HostedSettingsStore, org: string, stored: StoredClaude) {
  await store.setSecret(org, CLAUDE_ACCOUNT_SECRET, JSON.stringify(stored));
  await store.setSecret(org, CLAUDE_PENDING_SECRET, null);
}

async function refreshIfNeeded(store: HostedSettingsStore, org: string, stored: StoredClaude): Promise<StoredClaude> {
  ...
  try {
    ...
    await persist(store, org, merged);
    return merged;
  } catch (error) {
    if (error instanceof Error && /expired/i.test(error.message)) await store.setSecret(org, CLAUDE_ACCOUNT_SECRET, null);
    throw error;
  }
}

export async function claudeAccountStatus(store: HostedSettingsStore, org: string): Promise<HostedClaudeAccount> {
  const secrets = await store.secrets(org);
  const pending = parsePending(secrets[CLAUDE_PENDING_SECRET]);
  if (pending && pending.expiresAt <= Date.now()) await store.setSecret(org, CLAUDE_PENDING_SECRET, null);
  const stored = parseStored(secrets[CLAUDE_ACCOUNT_SECRET]);
  if (!stored) return publicClaudeAccount(await store.secrets(org));
  try {
    const fresh = await refreshIfNeeded(store, org, stored);
...
export async function startClaudeAccount(store: HostedSettingsStore, org: string): Promise<HostedClaudeAccount> {
  ...
  await store.setSecret(org, CLAUDE_PENDING_SECRET, JSON.stringify({
    state,
    codeVerifier,
...
export async function completeClaudeAccount(store: HostedSettingsStore, org: string, input: unknown): Promise<HostedClaudeAccount> {
  ...
  const pending = parsePending((await store.secrets(org))[CLAUDE_PENDING_SECRET]);
  ...
  await persist(store, org, stored);
  return publicAccount(stored, undefined, Date.now());
}

export async function cancelClaudeAccount(store: HostedSettingsStore, org: string): Promise<HostedClaudeAccount> {
  await store.setSecret(org, CLAUDE_PENDING_SECRET, null);
  return publicClaudeAccount(await store.secrets(org));
}

export async function logoutClaudeAccount(store: HostedSettingsStore, org: string): Promise<HostedClaudeAccount> {
  await store.setSecret(org, CLAUDE_ACCOUNT_SECRET, null);
  await store.setSecret(org, CLAUDE_PENDING_SECRET, null);
  return { phase: "signedOut" };
}

/// Refreshes the connected account and returns the file Claude Code reads.
/// Values stay out of management APIs; only computer bootstrap receives them.
export async function claudeComputerEnvironment(
  store: HostedSettingsStore,
  org: string,
